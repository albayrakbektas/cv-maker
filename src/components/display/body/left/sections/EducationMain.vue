<template>
  <div class="section">
    <CollapsibleSection title="Education">
      <CollapsibleContent>
        <form>
          <FormField
            :form-field="educationField"
            v-model="educationField.value"
          />
          <div class="row-two">
            <FormField :form-field="schoolField" v-model="schoolField.value" />
            <FormField :form-field="cityField" v-model="cityField.value" />
          </div>
          <div class="row-two">
            <div class="row-two">
              <FormField
                :form-field="startMonthField"
                v-model="startMonthField.value"
              />
              <FormField
                :form-field="startYearField"
                v-model="startYearField.value"
              />
            </div>
            <div class="row-two">
              <FormField
                :form-field="endMonthField"
                v-model="endMonthField.value"
              />
              <FormField
                :form-field="endYearField"
                v-model="endYearField.value"
              />
            </div>
          </div>
          <FormField
            :form-field="descriptionField"
            v-model="descriptionField.value"
          />
        </form>
      </CollapsibleContent>
    </CollapsibleSection>
  </div>
</template>

<script>
import CollapsibleSection from "@/components/collapse/CollapsibleSection";
import CollapsibleContent from "@/components/collapse/CollapsibleContent";
import FormField from "@/components/form/FormField";
export default {
  name: "EducationMain",
  components: { FormField, CollapsibleContent, CollapsibleSection },
  methods: {
    changeBodyCards() {
      this.$store.dispatch("changeBodyCards", this.card);
    },
  },
  watch: {
    "educationField.value": function (val) {
      this.$store.state.isChanging = !this.$store.state.isChanging;
      this.card.title = val;
      this.$store.state.education.cards[this.cardIndex] = this.card;
    },
    "schoolField.value": function (val) {
      this.$store.state.isChanging = !this.$store.state.isChanging;
      this.card.rowCard.subtitle[0] = val;
      this.$store.state.education.cards[this.cardIndex] = this.card;
    },
    "cityField.value": function (val) {
      this.$store.state.isChanging = !this.$store.state.isChanging;
      this.card.rowCard.subtitle[1] = val;
      this.$store.state.education.cards[this.cardIndex] = this.card;
    },
    "descriptionField.value": function (val) {
      this.$store.state.isChanging = !this.$store.state.isChanging;
      this.card.rowCard.content = val;
      this.$store.state.education.cards[this.cardIndex] = this.card;
    },
  },
  mounted() {
    this.cardIndex = this.$store.state.education.cards.length - 1;
  },
  data() {
    return {
      cardIndex: 0,
      card: {
        title: "Maths",
        dateStart: "October 2017",
        dateEnd: "June 2023",
        rowCard: {
          subtitle: ["s", "a"],
          content: "Description",
        },
      },
      educationField: {
        tag: "input",
        type: "text",
        name: "education",
        label: "Education",
        placeholder: "",
        value: "",
      },
      schoolField: {
        tag: "input",
        type: "text",
        name: "school",
        label: "School",
        placeholder: "",
        value: "",
      },
      cityField: {
        tag: "input",
        type: "text",
        name: "city",
        label: "City",
        placeholder: "",
        value: "",
      },
      startMonthField: {
        tag: "input",
        type: "text",
        name: "startMonth",
        label: "Start Date",
        placeholder: "",
        value: "",
      },
      startYearField: {
        tag: "input",
        type: "text",
        name: "startYear",
        label: "sy",
        placeholder: "",
        value: "",
      },
      endMonthField: {
        tag: "input",
        type: "text",
        name: "endMonth",
        label: "End Date",
        placeholder: "",
        value: "",
      },
      endYearField: {
        tag: "input",
        type: "text",
        name: "endYear",
        label: "ey",
        placeholder: "",
        value: "",
      },
      descriptionField: {
        tag: "textarea",
        type: "text",
        name: "description",
        label: "Description",
        placeholder: "",
        value: "",
      },
    };
  },
};
</script>

<style scoped lang="scss">
.row-two {
  display: grid;
  grid-template-columns: repeat(2, 1fr);
}
</style>
